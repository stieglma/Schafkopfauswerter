...
public privileged aspect StatisticsGlobalExport {
	...
	declare precedence: StatisticsGlobalExport, StatisticsExport, GraphExport, PdfExport;
	private GUI gui;
	 ...
	 * @throws IOException
	 */
	private void createOverallContent(PDPageContentStream contentStream)
			throws IOException {
		...
		contentStream.drawString("#Spiele (ohne weiter): "
				+ data.getAnzahlSpieleGesamt() + " (gewonnen: "
				+ String.format("%.2f", data.getGesamtWinPerc()) + "%)");

		contentStream.moveTextPositionByAmount(0, -16);
		contentStream.drawString("#Rufspiele: " + data.getAnzahlRufspiele()
				+ " (gewonnen: "
		...
		contentStream.moveTextPositionByAmount(-50, -16);
		contentStream.drawString("#Soli gesamt: " + data.getAnzahlSoliGesamt()
				+ " (gewonnen: "
		...
		contentStream.moveTextPositionByAmount(50, -16);
		contentStream.drawString("-> teuerstes Solo Sie: "
