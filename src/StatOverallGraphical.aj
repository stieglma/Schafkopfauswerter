...
public privileged aspect StatOverallGraphical {
	...
	private static PiePlot createPlayedTypesPlot() {
		...
		// Format plot
		plot.setSetting(PiePlot.TITLE, "Anzahl gespielter Spieltypen");
		// Change plot size
		...
		plot.getPointRenderer(playedTypesTable).setSetting(
				PieSliceRenderer.RADIUS_INNER, 0.4);
		// Change the width of gaps between segments
		plot.getPointRenderer(playedTypesTable).setSetting(PieSliceRenderer.GAP, 0.2);
		// Show labels
		plot.getPointRenderer(playedTypesTable).setSetting(
				PieSliceRenderer.VALUE_DISPLAYED, true);
		plot.getPointRenderer(playedTypesTable).setSetting(
				PieSliceRenderer.VALUE_COLOR, Color.WHITE);
		plot.getPointRenderer(playedTypesTable).setSetting(
