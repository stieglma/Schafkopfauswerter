...
public class SavingActionListener implements ActionListener {
  ...
  public SavingActionListener(GUI parent, AddGamePopup parentPopup, Schafkopfmodel model) {
    this.model = model;
    this.parent = parent;
  ...
  public void actionPerformed(ActionEvent e) {
    Games game =
        Games.values()[
            parentPopup.spielComboBox.getSelectedIndex()
                + 1]; // we need to exclude Games.NOONE here
    if (game == Games.WEITER) {
      ...
      if (laufende > 14 || laufende < 0 || gedoppelt < 0 || gedoppelt > 4) {
        JOptionPane.showMessageDialog(
            parentPopup, "Nur zahlen im richtigen Wertebereich zulässig.");
      }
      ...
      switch (game) {
        case RUFSPIEL:
          model.addRufspiel(
              p1,
              p2,
              parentPopup.spielerGewinntRadioButton.isSelected(),
              laufende,
              schneider,
              spritze,
              gedoppelt);
        ...
        case TOUT:
          model.addSoloTout(
              p1, parentPopup.spielerGewinntRadioButton.isSelected(), laufende, spritze, gedoppelt);
          break;
        case SIE:
